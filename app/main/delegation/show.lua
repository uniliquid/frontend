...
if member_id == app.session.member_id then
  edit = true
end
...
if unit then
  unit:load_delegation_info_once_for_member_id(member_id)
  ...
  if edit then
    ...
    ui.title((config.single_unit_id and _"View global delegation for '#{member_name}'" or _"View unit delegation for '#{member_name}'"):gsub("#{member_name}", member.name))
    util.help("delegation.new.unit.view")
  end
end

local area = Area:by_id(param.get("area_id", atom.integer))
if area then
  area:load_delegation_info_once_for_member_id(member_id)
  ...
  if edit then
    ...
    ui.title(_"View delegation for Area '#{name}' for Member '#{member_name}'":gsub("#{name}", area.name):gsub("#{member_name}", member.name))
    util.help("delegation.new.area.view")
  end
end

local issue = Issue:by_id(param.get("issue_id", atom.integer))
if issue then
  issue:load("member_info", { member_id = member_id })
  ...
  if edit then
    ...
    ui.title(_"View delegation for Issue ##{number} in Area '#{area_name}' for Member '#{member_name}'":gsub("#{number}", issue.id):gsub("#{area_name}", issue.area.name):gsub("#{member_name}", member.name))
    util.help("delegation.new.issue.view")
  end
end
...
if issue_id then
  ...
  delegation = Delegation:by_pk(member_id, nil, nil, issue_id)
  if not delegation then
    delegation = Delegation:by_pk(member_id, nil, issue.area_id)
  end
  if not delegation then
    delegation = Delegation:by_pk(member_id, issue.area.unit_id)
  end
elseif area_id then
  delegation = Delegation:by_pk(member_id, nil, area_id)
  if not delegation then
    local area = Area:by_id(area_id)
    delegation = Delegation:by_pk(member_id, area.unit_id)
  end
end

if not delegation then
  delegation = Delegation:by_pk(member_id, unit_id)
end
...
local contact_members = Member:build_selector{
  is_contact_of_member_id = member_id,
  voting_right_for_unit_id = voting_right_unit_id,
  active = true,
  locked = false,
  order = "name"
...
if edit or preview_trustee_id then
...
ui.form{
  ...
  content = function()
    ...
    if issue then
      ...
      local area_delegation = Delegation:by_pk(member_id, nil, issue.area_id)
      if area_delegation then
        delegate_name = area_delegation.trustee and area_delegation.trustee.name or _"abandoned"
        scope = _"area"
      else
        local unit_delegation = Delegation:by_pk(member_id, issue.area.unit_id)
    ...
    elseif area then
      local delegate_name = ""
      local scope = "no delegation set"
      local unit_delegation = Delegation:by_pk(member_id, area.unit_id)
      if unit_delegation then
        delegate_name = unit_delegation.trustee.name
        scope = config.single_unit_id and _"global" or _"unit"
      end
    ...
    disabled_records[member_id] = true
...
local delegation_chain = Member:new_selector()
  ...
  :join({ "delegation_chain(?,?,?,?,?,?)", member_id, unit_id, area_id, issue_id, tmp_preview_trustee_id, preview_inherit }, "delegation_chain", "member.id = delegation_chain.member_id")
  :add_order_by("index")
  :exec()
